<template>
  <div class="message-input">
    <!-- 输出框 -->
    <van-field v-model="message" :class="{'input-fixed': optionShow || emojiShow   }" rows="1" autosize type="textarea" left-icon="volume-o" placeholder="请输入" clearable autofocus center>
      <template #right-icon>
        <van-icon name="smile-o" size=".5rem" color="#000" @click="togglEmmoji" />
      </template>
      <template #button>
        <van-icon v-if="toggleButton" name="add-o" size=".5rem" color="#000" @click="toggleOption" />
        <van-button v-else type="primary" @click="sendMessage" size="mini">发送</van-button>
      </template>
    </van-field>
    ...
    <!-- 表情弹出框 -->
    <emoji-popup :emojiShow="emojiShow" @selectEmoji="selectOnEmoji"></emoji-popup>
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    watch([() => optionShow.value, () => emojiShow.value], ([newOtion, newEmoji], [oldOption, oldEmoji]) => {
